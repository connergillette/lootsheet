...
export default function Nav({ signOut, session }: Props) {
  ...
  return (
    <div className="fixed flex align-middle w-8/12 min-w-[900px] max-md:w-11/12 max-md:min-w-[300px] mx-auto h-10 z-20">
      <a href="/" className="py-1" onMouseEnter={onHover} onMouseLeave={() => setHoverColor('')}>
        ...
        }
        {
          session && (
            <>
              ...
              <button className={`hover:bg-gray-100 hover:text-black rounded-md px-4 py-1 transition h-min text-gray-600`} onClick={() => signOut()}>Log out</button>
            </>
          )
        }
      </div>
